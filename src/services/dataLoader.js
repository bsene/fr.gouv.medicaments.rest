...
async function parseFile(filename, columns) {
  const filepath = path.join(dirname, DATA_DIR, filename);
  ...
  try {
    ...
    try {
      ...
      console.log(
        `✓ Parsing ligne par ligne réussi pour ${filename}: ${records.length} entrées`,
      );
      return records;
    } catch (fallbackError) {
      console.error(
        `Erreur parsing fallback ${filename}:`,
...
export async function loadData() {
  ...
  try {
    const stats = Deno.statSync(mainFilePath);
  ...
  dataCache.specialites = await parseFile("CIS_bdpm.txt", [
    "cis",
    "denomination",
    "forme_pharma",
    "voies_admin",
  ...
  dataCache.presentations = await parseFile("CIS_CIP_bdpm.txt", [
    "cis",
    "cip7",
    "libelle",
    "statut_admin",
...
export function searchInData(dataset, query, fields) {
